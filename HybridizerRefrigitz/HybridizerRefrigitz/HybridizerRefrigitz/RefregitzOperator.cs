    ...
    public class RefregitzOperator//:RefregizMemmory
    {
            ...
            )
        {
            ...
            lock (o)
            {
                ...
                HybridizerRefrigitz.AllDraw Current = new HybridizerRefrigitz.AllDraw(Order, MovementsAStarGreedyHeuristicFoundT, IgnoreSelfObjectsT, UsePenaltyRegardMechnisamT, BestMovmentsT, PredictHeuristicT, OnlySelfT, AStarGreedyHeuristicT, ArrangmentsT);
            }
        }


        public HybridizerRefrigitz.AllDraw GetRefregiz(int No)
        {
            Object o = new Object();
            lock (o)
            {
                ...
                int p = 0;
                HybridizerRefrigitz.AllDraw Dummy = null;
                ...
                DummyFileStream.Seek(0, SeekOrigin.Begin);
                    ...
                    while (p <= No)
                    {
                        if (DummyFileStream.Length >= DummyFileStream.Position)
                            Dummy = (HybridizerRefrigitz.AllDraw)Formatters.Deserialize(DummyFileStream);
                        else
                            Dummy = null;
                        p++;
                    }
